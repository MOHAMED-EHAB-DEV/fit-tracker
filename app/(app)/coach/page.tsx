...
const MODEL_OPTIONS: { id: GeminiModelChoice; label: string; tag: string; description: string }[] = [
  {
...
export function CoachPage() {
  ...
  const selectCommand = (cmd: CommandDef, customDays?: number) => {
    ...
    if (cmd.dayOptions && cmd.dayOptions.length > 0) {
      setInput("");
    } else {
      setInput("");
    }
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    // If there is an active command selected (Discord style)
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto rounded-2xl bg-zinc-900/80 border border-zinc-800/80 overflow-hidden shadow-2xl relative">
      ...
      <div className="p-4 border-b border-zinc-800/80 bg-zinc-950/70 flex items-center justify-between backdrop-blur-sm z-10">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-zinc-900 border border-zinc-700/70 hover:border-emerald-500/40 text-xs text-zinc-200 transition shadow-inner"
          >
            <Cpu className="w-3.5 h-3.5 text-emerald-400" />
            <span className="font-medium">
              {MODEL_OPTIONS.find((m) => m.id === selectedModel)?.label || "Gemini Flash"}
            </span>
            <ChevronDown className={cn("w-3.5 h-3.5 text-zinc-400 transition-transform", isModelDropdownOpen && "rotate-180")} />
          </button>
            ...
            <div className="absolute right-0 mt-2 w-72 rounded-xl bg-zinc-950 border border-zinc-800 shadow-2xl p-2 z-50 space-y-1">
              ...
              {MODEL_OPTIONS.map((opt) => (
                <button
                  ...
                  className={cn(
                    "w-full text-left p-2.5 rounded-lg text-xs transition flex flex-col gap-0.5",
                  ...
                  <div className="p-3 rounded-xl bg-zinc-950/60 border border-emerald-500/20 space-y-1.5 text-xs">
                    ...
                    <div className="space-y-1">
                      {msg.logItems.map((item, i) => (
                        ...
                        >
                          {item.type === "meal" && <UtensilsCrossed className="w-3.5 h-3.5 text-orange-400 shrink-0" />}
                          {item.type === "water" && <Droplets className="w-3.5 h-3.5 text-cyan-400 shrink-0" />}
                          {item.type === "steps" && <Footprints className="w-3.5 h-3.5 text-blue-400 shrink-0" />}
            ...
            {filteredCommands.map((cmd, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full text-left px-3 py-2.5 rounded-xl transition flex items-center justify-between group",
                    isSelected
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <CornerDownLeft className="w-3.5 h-3.5 text-zinc-500 group-hover:text-emerald-400 opacity-0 group-hover:opacity-100 transition" />
                  </div>
                </button>
              );
            })}
        ...
        <div className="px-4 py-2 bg-zinc-950 border-t border-zinc-800/80 flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-2.5 py-1 rounded-lg bg-emerald-500 text-zinc-950 font-bold flex items-center gap-1.5 shadow-sm">
              <activeCommand.icon className="w-3.5 h-3.5" />
              {activeCommand.name}
            </span>

            {/* Day Option Chips (like Discord parameter options) */}
              ...
              <div className="flex items-center gap-1.5">
                ...
                {activeCommand.dayOptions.map((d) => (
                  <button
                    ...
                    className={cn(
                      "px-2 py-0.5 rounded-md font-semibold text-[11px] transition",
                      selectedDays === d
      ...
      <div className="p-3 border-t border-zinc-800/80 bg-zinc-950 flex items-center gap-2">
        <input
          ...
          className="flex-1 px-4 py-3 bg-zinc-900 border border-zinc-700/60 rounded-xl text-white placeholder-zinc-500 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
        />
        <button
          type="button"
          onClick={() => sendMessage()}
          disabled={(!input.trim() && !activeCommand) || isLoading}
          className="p-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold transition disabled:opacity-50 flex items-center justify-center shadow-lg shadow-emerald-500/10"
        >
