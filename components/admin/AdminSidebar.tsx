...
export function AdminSidebar({ adminName, adminEmail }: AdminSidebarProps) {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-screen bg-zinc-950 border-r border-violet-900/30 p-5 sticky top-0 shrink-0 select-none">
      ...
      <Link
        href="/"
        className="flex items-center gap-2 px-3 py-2 mb-4 rounded-xl text-xs text-zinc-500 hover:text-violet-400 hover:bg-violet-500/5 transition-all group"
      >
        ...
        {ADMIN_NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <div className="w-1.5 h-5 bg-violet-400 rounded-full absolute right-2" />
              )}
            </Link>
          );
        })}
      ...
      <div className="pt-4 border-t border-violet-900/30 space-y-2">
        <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-violet-950/40 border border-violet-800/30">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-8 h-8 rounded-full bg-violet-500/20 border border-violet-500/40 text-violet-400 flex items-center justify-center font-bold text-xs shrink-0">
