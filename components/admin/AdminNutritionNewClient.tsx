...
export function AdminNutritionNewClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !targetCalories) {
      setError("Name and target calories are required.");
      return;
    }
    setIsLoading(true);
    setError("");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
      ...
      <div className="bg-zinc-900/60 border border-zinc-800/60 rounded-2xl p-6 space-y-4">
        ...
        <div>
          <label htmlFor="plan-name" className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1 select-none">
          ...
          <input
            ...
            placeholder="e.g. 2500kcal Muscle Building Plan"
            value={name}
            onChange={(e) => setName(e.target.value)}
          />
        </div>
        <div>
          <label htmlFor="plan-desc" className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1 select-none">
            Description
          </label>
          <textarea
            id="plan-desc"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <label htmlFor="plan-calories" className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1 select-none">
              Target Calories *
            </label>
            <input
              id="plan-calories"
          ...
          <div>
            <label htmlFor="plan-protein" className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1 select-none">
              Protein (g)
            </label>
            <input
              id="plan-protein"
          ...
          <div>
            <label htmlFor="plan-carbs" className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1 select-none">
              Carbs (g)
            </label>
            <input
              id="plan-carbs"
          ...
          <div>
            <label htmlFor="plan-fat" className="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1 select-none">
              Fat (g)
            </label>
            <input
              id="plan-fat"
      ...
      <div className="flex justify-end gap-3 pb-8">
        <button
          type="button"
          onClick={() => router.back()}
          className="px-4 py-2.5 min-h-[44px] text-sm text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900 rounded-xl transition cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400"
        >
          Cancel
        </button>
        <button
          ...
          disabled={isLoading}
          aria-busy={isLoading}
          className="flex items-center gap-2 px-5 py-2.5 min-h-[44px] bg-violet-500 text-white text-sm font-semibold rounded-xl hover:bg-violet-600 transition shadow-lg shadow-violet-500/20 disabled:opacity-60 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-400"
        >
...
export default AdminNutritionNewClient;
