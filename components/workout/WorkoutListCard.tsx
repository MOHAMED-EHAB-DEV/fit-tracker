...
export function WorkoutListCard({ workout }: WorkoutListCardProps) {
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-[26px] bg-zinc-900/80 backdrop-blur-2xl border border-white/10 hover:border-white/20 transition-all shadow-xl group relative">
        <Link
          href={`/workouts/${workout.id}`}
          className="flex items-center gap-3.5 flex-1 min-w-0"
        >
          <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold shrink-0 group-hover:scale-105 transition-transform">
