...
import { Scale, Camera, Plus, Sparkles, Calendar, Loader2, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 min-h-[44px] bg-zinc-950 border border-zinc-700/60 rounded-xl text-white placeholder-zinc-500 tabular-nums focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
            />
          </div>

          <div>
            <label htmlFor="bodycomp-bf" className="block text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-1.5 select-none">
        ...
        {/* Photo Upload */}
        <div>
          <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-1.5 select-none">
          ...
          {previewUrl ? (
            <div className="relative w-36 h-48 rounded-xl overflow-hidden border border-zinc-700 bg-zinc-950 group">
              ...
              <button
                ...
                className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-xs font-bold text-white transition cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
              >
                Change
              </button>
            </div>
            ...
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="px-4 py-3 min-h-[44px] rounded-xl bg-zinc-950 border border-dashed border-zinc-700 hover:border-emerald-500/50 text-zinc-300 text-xs font-semibold flex items-center gap-2 transition cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
            >
        ...
        <button
          ...
          aria-busy={isSubmitting || isResizing}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {item.bodyFatPercent && (
                        <span className="text-xs font-semibold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 tabular-nums">
